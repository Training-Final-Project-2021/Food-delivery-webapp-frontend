...
function Deliver() {
    ...
    const handleDeliver = (order_id) => {
        const url = `http://localhost:3030/v1/deliveries/deliver_order`
        axios.put(url, { order_id }, { headers: { "AUTH-TOKEN": localStorage.getItem("delivery_auth_token") } })
            .then(response => {
                //console.log(response.data.orders);
                if (response.data.is_success) {
                    //setState({ ...state, orders: response.data.orders })
                                ...
                                {state.orders.map(order => {
                                    return (
                                        <tr key={order.id}>
                                            ...
                                            <td><button className="btn btn-warning" onClick={()=> handleDeliver(order.id)}>Deliver</button></td>
